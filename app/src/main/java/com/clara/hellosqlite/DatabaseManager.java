...
public class DatabaseManager {
	...
	public int getQuantityForProduct(String productName){
		...
		String[] slectionArgs={productName};
		Cursor cursor = db.query(DB_TABLE, cols,selection,slectionArgs,null,null,null);
	...
	public boolean updateQuantity(String name, int newQuantity){
		...
		int rowsChaged=db.update(DB_TABLE,updateProduct,where,whereArgs);
		Log.i(DB_TAG,"Update "+ name+ " new quantity" + newQuantity + " rows modified "+ rowsChaged );
		if (rowsChaged>0){
			return true;
		}
		return false;
	...
	//TO add method to fetch all data and return a Cursor
	//TOD add method to select (search) for a product by name
	//TOO add method to insert (add) a product and quantity
	//TOD add method to delete a product
	//TOD add method to update (change) the quantity of a product
